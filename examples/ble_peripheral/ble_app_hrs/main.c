...
#define ADC_MEAS_INTERVAL        APP_TIMER_TICKS(1000, APP_TIMER_PRESCALER)
...
//    nrf_adc_conversion_event_clean();
	...
	//  nrf_gpio_pin_set(LED_PIN_2);//进入定时器时事件时关闭led2
	
    adc_sample = nrf_adc_result_get();
	  
	
	  index=sizeof(adc_sample);
	
   	err_code = ble_lbs_on_adc_send(&m_adc,&adc_sample,index);
...
static void timers_init(void)
{
    ...
    APP_ERROR_CHECK(err_code);
		err_code = app_timer_create(&m_adc_get_timer_id,
                                APP_TIMER_MODE_REPEATED,
                                adc_get_handler);
    APP_ERROR_CHECK(err_code);
}
